...
//        return "NULL";
    ...
    public void ErpPopupFetchEvent(PopupFetchEvent erppfe) {
        ...
        vo.setWhereClause("EXISTS (SELECT '' FROM SYS_USERS_MODULE SUM WHERE SUM.MODULE_ACTION=QRSLT.MODULE_ACTION AND  SUM.USER_ID="+ERPGlobalsClass.doGetUserSno()+") AND upper(SEARCH_STRING) like UPPER('%" +
                          (strglobalsearch.length() == 0 ? "###-###" : strglobalsearch) + "%')");
